...
purchases <- read.delim('/home/hadoop/data/purchases.txt', header=FALSE)

#purchases.map <- function(k, v){
#  city <- v[[3]]
#  dollars <- v[[5]]
#  dat <- data.frame(city, dollars)
#  z <- aggregate(dollars ~ city, dat, FUN = sum)
#  keyval(z[[1]], z[[2]])
#}
...
m = mapreduce(input='/user/purchases/data', output='/user/purchases/out18', input.format=purchases.format,
